...
[verbose,optTol,progTol,maxIter,memory,testOpt,curvilinear] = myProcessOptions(options,'verbose',2,'optTol',1e-5,...
    'progTol',1e-9,'maxIter',500,'memory',10,'testOpt',1,'curvilinear',0);
...
for i = 1:maxIter
	...
	if curvilinear
		x_new = funProj(x-t*alpha*g,t*alpha);
	else
    ...
    while f_new > fr || ~isLegal(f_new)
        ...
        if max(abs(x_new-x)) < progTol || t == 0
            if verbose == 3
